	...
	onMount(async () => {
		...
		function onScroll() {
			...
			if (y < 10) {
				...
				// Skip — likely Chrome address bar animation
					...
					<li class:pinned={place.Pinned}>
						...
						{#each tags(place) as tag}
							<span class="tag-wrap">
								...
								>{tag.value}</span>
								{#if !readonly && editingTag && editingTag.placeId === place.id && editingTag.field === tag.field && editingTag.index === tag.index}
	...
	{#if viewMode === 'map'}
		{#if browser}
			<MapView
				...
				city={$selectedCity}
				onTogglePin={readonly ? null : togglePin}
			/>
		{/if}
	{:else}
		<section>
	...
	<div class="modal-overlay">
		<div class="modal">
			<header>
				...
				{#if duplicateWarning}
					<p class="duplicate-warning">Warning: "{duplicateWarning}" already exists in your list</p>
				{/if}
